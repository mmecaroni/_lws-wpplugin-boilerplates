...
function lws_wpsettings_boilerplate_enqueue_script_htmx() {
	...
	if (file_exists($local_script_path)) {
			wp_enqueue_script(
				'lws-boilerplate-htmx', $local_script_url, array(), '1.9.10', true
			);
	} else {
			wp_enqueue_script(
				'lws-boilerplate-htmx', $cdn_script_url, array(), '1.9.10', true
			);
	}
